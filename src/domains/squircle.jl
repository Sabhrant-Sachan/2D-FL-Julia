...
"""
    boundquad!(P::SubArray{Float64}, d::squircle, k::Int) -> nothing

Find a bounding quadrilateral for the k-th patch.

Mutates `P` into `[x1,y1,x2,y2,x3,y3,x4,y4]` in counter-clockwise order.

If the patch is already a quadrilateral, return its four corners directly.
Otherwise:
1. Build the two opposite sides `u = -1` and `u = +1`.
2. Along each side, intersect the tangent line at `τ(u,t)` with the
   two straight sides to find extremal intersections.
3. Assemble the bounding quadrilateral.
"""
function boundquad!(P::SubArray{Float64}, d::squircle, k::Int)
